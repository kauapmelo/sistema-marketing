...
function Avatar({ member, size=28, style={} }) {
  return <div style={{ width:size, height:size, borderRadius:"50%", background:member.color, color:"#fff", display:"flex", alignItems:"center", justifyContent:"center", fontSize:size*.36, fontWeight:700, flexShrink:0, border:`2px solid ${T.bg1}`, boxSizing:"border-box", ...style }}>{member.avatar||initials(member.name)}</div>;
}
  ...
  const addComment = () => {
    ...
    (form.members||[]).forEach(mid => {
      if(mid !== currentUser?.id) {
        ...
        if(mb) onNotify(mb.id, `${currentUser?.name||"Alguém"} comentou em "${form.title}"`);
      }
    });
  };
          ...
          <div style={{ flex:1,padding:"20px 24px",borderRight:`1px solid ${T.border}` }}>
            ...
            {totalChecks>0 && (
              <div style={{ background:T.bg3,borderRadius:6,height:6,marginBottom:10 }}>
                <div style={{ background:T.green,borderRadius:6,height:6,width:`${(doneChecks/totalChecks)*100}%`,transition:"width .3s" }} />
              </div>
            )}
            ...
            <div style={{ display:"flex",gap:8 }}>
              <input value={comment} onChange={e=>setComment(e.target.value)} onKeyDown={e=>e.key==="Enter"&&addComment()} placeholder="Escrever comentário..." style={s.input({flex:1})} />
    ...
    <div draggable onDragStart={e=>{setDrag(true);e.dataTransfer.setData("card",JSON.stringify({card,fromCol:colId}));}} onDragEnd={()=>setDrag(false)}
      ...
      <div style={{ display:"flex",justifyContent:"space-between",alignItems:"center" }}>
        <div style={{ display:"flex" }}>
          {cardMembers.map((m,i)=><div key={m.id} style={{ marginLeft:i?-8:0 }} title={m.name}><Avatar member={m} size={22} /></div>)}
        </div>
...
function BoardTab({ columns, updateColumns, members, currentUser, onNotify, taskTypes, updateTaskTypes }) {
  ...
  const filterCard = card => {
    ...
    if(filterMember!=="all" && !(card.members||[]).includes(+filterMember)) return false;
    return true;
  };
      ...
      {editing && (
        <div style={{ position:"fixed",inset:0,background:"rgba(0,0,0,.75)",display:"flex",alignItems:"center",justifyContent:"center",zIndex:1000 }} onClick={e=>e.target===e.currentTarget&&setEditing(null)}>
          <div style={s.card({padding:28,width:380,boxShadow:"0 24px 64px #000000cc"})}>
            ...
            {[["Nome","name","text"],["Cargo","role","text"]].map(([l,k,t])=>(
...
function AnalyticsTab({ columns, members }) {
  ...
  const allCards = columns.flatMap(c=>c.cards||[]);
  const doneCards = (columns.find(c=>c.id==="done")?.cards)||[];
  const filtered = filter==="all" ? doneCards : doneCards.filter(c=>(c.members||[]).includes(+filter));
  ...
  const kpis = [
    ...
    ["Pontos",filtered.reduce((a,c)=>a+c.points,0),T.amber,"⭐"],
  ];
    ...
    <div>
      ...
      <div style={{ display:"grid",gridTemplateColumns:"1fr 1fr",gap:20,marginBottom:20 }}>
        <div style={s.card({padding:20})}>
          <h3 style={{ margin:"0 0 16px",fontSize:15,fontWeight:700,color:T.text }}>Tarefas mais realizadas</h3>
...
function SocialTab({ data, updateData }) {
  ...
  const handleCSV = (e) => {
    ...
    reader.onload = ev => {
      ...
      if(rows.length) updateData({...data,[platform]:[...(data[platform]||[]),...rows]});
  ...
  const posts = [...(data[platform]||[])].sort((a,b)=>b[sortBy]-a[sortBy]);
      ...
      <div style={{ display:"flex",flexDirection:"column",gap:14 }}>
        {posts.length===0 && (
          <div style={s.card({padding:40,textAlign:"center"})}>
            ...
            <p style={{ color:T.textMuted,fontSize:14 }}>Nenhum dado para esta plataforma.<br/>Importe um CSV para começar.</p>
          </div>
        )}
        {posts.map((post,rank)=>{
          ...
          return (
            <div key={post.id} style={s.card({padding:20,display:"flex",gap:20,alignItems:"flex-start"})}>
              ...
              <button onClick={()=>updateData({...data,[platform]:data[platform].filter(p=>p.id!==post.id)})} style={{ background:"none",border:"none",cursor:"pointer",color:T.textMuted,fontSize:16,padding:0 }}>🗑️</button>
            </div>
          );
        })}
      </div>
          ...
          {Array.from({length:days}).map((_,i)=>{
            ...
            return (
              <div key={day} onClick={()=>setSel(day===sel?null:day)} style={{ minHeight:90,borderBottom:`1px solid ${T.border}`,borderRight:`1px solid ${T.border}`,padding:"8px 6px",cursor:"pointer",background:isSel?T.accentDim:isToday(day)?T.blueDim:T.bg2,transition:"background .15s" }}>
                ...
                {evs.map(ev=>(
                  <div key={ev.id} style={{ background:memberColor(ev.memberId)+"33",borderLeft:`2px solid ${memberColor(ev.memberId)}`,borderRadius:"0 4px 4px 0",padding:"1px 5px",marginBottom:2,fontSize:10,fontWeight:600,color:memberColor(ev.memberId),overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap" }}>{ev.title}</div>
                ))}
                {dueCards.map(c=>(
                  <div key={c.id} style={{ background:T.amber+"22",borderLeft:`2px solid ${T.amber}`,borderRadius:"0 4px 4px 0",padding:"1px 5px",marginBottom:2,fontSize:10,fontWeight:600,color:T.amber,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap" }}>⭐ {c.title}</div>
                ))}
              </div>
            );
          })}
      ...
      {sel && (
        <div style={s.card({marginTop:16,padding:16})}>
          <div style={{ display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:12 }}>
            ...
            <button onClick={()=>setAddModal(dateStr(sel))} style={s.btn(T.accent,{fontSize:12,padding:"6px 12px"})}>+ Adicionar evento</button>
          </div>
          {eventsFor(sel).map(ev=>{
            ...
            return (
              <div key={ev.id} style={{ display:"flex",alignItems:"center",gap:12,padding:"8px 0",borderBottom:`1px solid ${T.border}` }}>
                ...
                <button onClick={()=>updateEvents(events.filter(e=>e.id!==ev.id))} style={{ background:"none",border:"none",cursor:"pointer",color:T.textMuted,fontSize:16 }}>🗑️</button>
              </div>
            );
          })}
          ...
          {eventsFor(sel).length===0 && allCards.filter(c=>c.due===dateStr(sel)).length===0 && (
            <p style={{ color:T.textMuted,fontSize:13,textAlign:"center",padding:"16px 0" }}>Nenhum evento neste dia</p>
          )}
        </div>
      )}
